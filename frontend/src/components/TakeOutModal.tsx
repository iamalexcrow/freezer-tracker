      ...
      <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-sm overflow-hidden">
        ...
        <div className="p-4 sm:p-6 space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <button
                type="button"
                onClick={() => {
                  const step = item.measuring_unit === "kg" ? 0.1 : 1;
                  const newVal = Math.max(0.1, parsedAmount - step);
                  setAmount(item.measuring_unit === "kg" ? newVal.toFixed(1) : String(Math.round(newVal)));
                }}
              ...
              <button
                ...
                onClick={() => {
                  const step = item.measuring_unit === "kg" ? 0.1 : 1;
                  const newVal = Math.min(item.amount, parsedAmount + step);
                  setAmount(item.measuring_unit === "kg" ? newVal.toFixed(1) : String(Math.round(newVal)));
                }}
                className="w-10 h-10 sm:w-12 sm:h-12 bg-gray-100 rounded-xl text-gray-700 font-bold hover:bg-gray-200 transition-colors text-lg sm:text-xl"
              >
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => setAmount(String(item.amount * 0.25))}
            ...
            <button
              type="button"
              onClick={() => setAmount(String(item.amount * 0.5))}
              className="flex-1 py-2 bg-gray-100 rounded-lg text-xs sm:text-sm font-medium text-gray-700 hover:bg-gray-200"
            >
              50%
            </button>
            <button
              type="button"
              onClick={() => setAmount(String(item.amount * 0.75))}
              className="flex-1 py-2 bg-gray-100 rounded-lg text-xs sm:text-sm font-medium text-gray-700 hover:bg-gray-200"
            >
              75%
            </button>
          ...
          {!isFullAmount && parsedAmount > 0 && (
            <div className="bg-amber-50 text-amber-700 px-3 sm:px-4 py-2 sm:py-3 rounded-xl text-xs sm:text-sm">
              <strong>Splitting:</strong> {parsedAmount.toFixed(item.measuring_unit === "kg" ? 1 : 0)} {item.measuring_unit} will be taken out.
              Remaining {(item.amount - parsedAmount).toFixed(item.measuring_unit === "kg" ? 1 : 0)} {item.measuring_unit} stays in freezer.
            </div>
          )}
